...
const client = new Client({
  ...
  intents: [Intents.FLAGS.GUILDS, Intents.FLAGS.GUILD_MESSAGES, Intents.FLAGS.GUILD_MEMBERS, Intents.FLAGS.DIRECT_MESSAGES, Intents.FLAGS.GUILD_BANS,Intents.FLAGS.GUILD_MESSAGE_REACTIONS],
});
...
client.on('debug', (debug) => {
  if (debug.includes("429")) {
    console.error("Too many requests")

    fs.appendFile('err', '429 '+Date.now()+'\n', function(err) {if (err) throw err;});
    spawn("kill", ["1"]) //kills docker resulting in ip change
  }
});
client.on('warn', (warn) => {
...
client.on("messageCreate", async message => {
  ...
  if (message.content === "<@" + client.user.id + ">" || message.content === "<@!" + client.user.id + ">") {
    message.reply("Woof! Type </help:1040999476405481534> to get started")
  }
...
client.on('messageUpdate', (oldMessage, message) => {
  ...
  client.channels.cache.get(channels["msgLog"]).send({
    embeds: [{
...
client.on("messageDelete", (message) => {
  if (message.author.bot==true) { return }
  console.log("Message deleted by "+message.author.id+" in "+message.channel.id+"\n"+message.content)
  if (message.content.length > 1024) { return }
  const channel = client.channels.cache.get(channels["msgLog"])
...
//I know require() is better to use, but eval allows me to edit specific parts of code without having to restart
...
eval(fs.readFileSync("./scripts/reactionManager.js", "utf-8"))
...
process.on('uncaughtException', function(exception) {
  ...
  if (client.cache === null || client.cache === undefined) return;
